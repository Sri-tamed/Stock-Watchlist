...
const StockInput: React.FC<StockInputProps> = ({ onAddStock, error, clearError, isAdding }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (symbol.trim()) {
      onAddStock(symbol);
      setSymbol(''); // Clear input on submission attempt
    }
  };
  ...
  return (
    <div className="p-6 bg-slate-800/50 rounded-lg shadow-lg border border-slate-700">
        ...
        <input
            ...
            className="flex-grow w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-colors placeholder-slate-500 uppercase"
            maxLength={5}
        ...
        <button
            ...
            disabled={!symbol.trim() || isAdding}
        >
            {isAdding ? 'Adding...' : 'Add Symbol'}
        </button>
        </form>
        {error && <p id="stock-error" role="alert" className="text-red-400 mt-3 text-sm">{error}</p>}
    </div>
  );
};
